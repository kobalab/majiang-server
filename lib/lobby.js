...
class Lobby {
    ...
    connect(sock) {
        ...
        catch (e) { console.error(e.stack); this.dump() }
    }
    ...
    dump() {
        console.log('== ROOM ==');
        for (let room_no of Object.keys(this.ROOM)) {
            console.log(this.ROOM[room_no].game ? ' *' : ' -', room_no,
                        this.ROOM[room_no].uids.map(uid=>uid.slice(-12)));
        }
        console.log('-- USER --');
        for (let uid of Object.keys(this.USER)) {
            console.log(this.USER[uid].sock ? ' *' : ' -', uid.slice(-12),
                        this.USER[uid].user.name, this.USER[uid].room_no || '');
        }
    }
}
