...
module.exports = class Lobby {
    ...
    connect(sock) {
        ...
        if (! this.USER[user.uid]) {
            ...
            if (this.ROOM[room_no].game) this.ROOM[room_no].game.connect(sock);
    ...
    enter_room(sock, room_no) {
        ...
        if (this.USER[user.uid].room_no) {
            sock.emit('ERROR', '既に入室済みです');
            sock.disconnect(true);
        }
        else if (! this.ROOM[room_no]) {
    ...
    start(sock, room_no, rule, timer) {
        ...
        this.ROOM[room_no].game
            = new Game(this.get_socks(room_no), callback, rule, null, timer);
