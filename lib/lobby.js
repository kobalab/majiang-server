...
function get_user(sock) {
    ...
    user.uid = user.uid ??
                    crypto.hash('sha256', session_id, 'hex').slice(0, 32);
    return user;
}
...
class Lobby {
        ...
        const callback = (paipu)=>{
            ...
            if (this.callback
                && ! Object.keys(this.ROOM)
                        .filter(r => this.ROOM[r].game).length) this.callback();
        };
        this.ROOM[room_no].game = new Game(this.get_socks(room_no), callback,
                                            rule, null, timer);
        this.ROOM[room_no].game.speed = this.nowait ? 0 : 2;
    ...
    close(callback) {
        ...
        console.log('** closing...');
        this.callback = ()=>{
            for (let sock of Object.keys(this.USER)
                                .map(uid => this.USER[uid].sock).filter(s => s))
            {
                sock.emit('ERROR', 'メンテナンスが完了しました');
            }
            callback();
        };

        for (let room_no of Object.keys(this.ROOM)) {
            if (this.ROOM[room_no].game) continue;
            for (let uid of this.ROOM[room_no].uids) {
                delete this.USER[uid].room_no;
                let sock = this.USER[uid].sock;
                if (sock) {
        ...
        for (let sock of Object.keys(this.USER)
                            .map(uid => this.USER[uid].sock).filter(s => s))
        {
            sock.emit('ERROR', 'メンテナンスを開始します');
        }
        if (! Object.keys(this.ROOM)
                .filter(r => this.ROOM[r].game).length) this.callback();
    }
    ...
    status(refresh, all, debug) {
        ...
        const game = Object.keys(this.ROOM).map(room_no => this.ROOM[room_no])
                            .filter(room => room.game)
                            .map(room => room.uids.filter(
                                        uid => this.USER[uid].sock).length)
                            .reduce((x, y)=> x + y, 0);

        let html = `<title>${title} (${game})</title>\n`
        ...
        let status = this.callback ? '【閉塞中】' : '';
        html += `<ul><li>${this.short_status() + status}</li></ul>\n`;
