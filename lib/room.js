...
function room_info(no) {
    ...
    for (let uid of ROOM[no].user) {
        ...
        sock.emit('ROOM', {
            no:   no,
            user: ROOM[no].user.map(uid=> USER[uid].user)
        });
    }
}
