...
function masking(id, qijia, logs) {
    for (let log of logs) {
        let menfeng;
        for (let msg of log) {
            if (msg.qipai) {
                menfeng = (id + 4 - qijia + 4 - msg.qipai.jushu) % 4
                for (let l = 0; l < 4; l++) {
                    if (l != menfeng) msg.qipai.shoupai[l] = '';
                }
            }
            else if (msg.zimo) {
                if (msg.zimo.l != menfeng) msg.zimo.p = '';
            }
            else if (msg.gangzimo) {
                if (msg.gangzimo.l != menfeng) msg.gangzimo.p = '';
            }
        }
    }
    return logs;
    ...
    connect(sock) {
        ...
        if (this._seq) {
            let msg = { kaiju: {
                ...
                log:    masking(id, this._paipu.qijia, this._paipu.log)
            } };
            sock.emit('GAME', msg);
        }
